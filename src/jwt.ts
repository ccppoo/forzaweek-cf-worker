...
import type { AuthInfo, JWTPayloadCustom } from '@/types';
import { Env } from '@/env';
...
function getAuthToken(request: Request): string | void {
	const authorization = request.headers.get('Authorization');
	if (!authorization) {
		return;
	}
	const [scheme, encoded] = authorization.split(' ');
	if (!encoded || scheme.toLocaleLowerCase() !== 'bearer') {
		return;
	}
	return encoded;
}
...
export async function verifyAuthentication(
	...
	try {
		...
		const { payload, protectedHeader } = await jose.jwtVerify<JWTPayloadCustom>(jwt_token, JWKS, {
